    ...
    <div className="aspect-[24/24] grow min-h-px min-w-px overflow-clip relative shrink-0">
      <div className="absolute aspect-[24/16] left-0 right-0 top-1/2 -translate-y-1/2">
        <svg className="block size-full" fill="none" viewBox="0 0 24 16">
          <g>
            <path d={svgPaths.p36880f80} className="fill-primary" />
            <path d={svgPaths.p355df480} className="fill-primary" />
            <path d={svgPaths.pfa0d600} className="fill-primary" />
          </g>
        </svg>
      </div>
    </div>
    ...
    <div className="relative shrink-0 w-full">
      <div className="flex items-center p-1 w-full">
        ...
        <div className="px-2 py-1">
          <div className="font-['Lexend:SemiBold',_sans-serif] text-[16px] text-primary">
            Interfaces
          </div>
        </div>
      </div>
...
function AvatarCircle() {
  return (
    <div className="relative rounded-full shrink-0 size-8 bg-white">
      ...
      >
        ...
        >
          <div className="size-8 flex items-center justify-center">
            <SearchIcon size={16} className="text-slate-900" />
          </div>
        </div>
          ...
          <div className="flex flex-col justify-center size-full">
            <div className="flex flex-col gap-2 items-start justify-center pr-2 py-1 w-full">
              <input
                ...
                className="w-full bg-transparent border-none outline-none font-['Lexend:Regular',_sans-serif] text-[14px] text-slate-900 placeholder:text-slate-500 leading-[20px]"
      ...
      sections: [
        {
          ...
          items: [
            {
              icon: <UserIcon size={16} className="text-slate-900" />,
              label: "Profile",
              hasDropdown: true,
              children: [
                { label: "Edit Profile", path: "/dashboard/edit-profile", isActive: currentPath === "/dashboard/edit-profile" },
              ]
            },
            {
              icon: <Security size={16} className="text-slate-900" />,
              label: "Security",
              hasDropdown: true,
              children: [
  ...
  return (
    <aside className="bg-white flex flex-col gap-2 items-center p-4 w-16 h-screen sticky top-0 border-r border-slate-200 rounded-l-2xl overflow-hidden">
      ...
      <div className="flex flex-col gap-2 w-full items-center">
        ...
        <IconNavButton onClick={() => {} /* handle logout */}>
    ...
    return (
      <div className="w-full flex justify-center transition-all duration-500" style={{ transitionTimingFunction: softSpringEasing }}>
        <button
          ...
          className="flex items-center justify-center rounded-lg size-10 min-w-10 transition-all duration-500 hover:bg-slate-100 text-slate-500 hover:text-slate-600"
          style={{ transitionTimingFunction: softSpringEasing }}
    ...
    <div className="w-full overflow-hidden transition-all duration-500" style={{ transitionTimingFunction: softSpringEasing }}>
      <div className="flex items-center justify-between">
        <div className="flex items-center h-10">
          <div className="px-2 py-1">
            <div className="font-['Lexend:SemiBold',_sans-serif] text-[18px] text-primary leading-[27px]">
              {title}
            </div>
          </div>
        </div>
...
function DetailSidebar({ activeSection, role }: { activeSection: string; role?: string | null }) {
  ...
  return (
    ...
    >
      ...
      {!isCollapsed && (
        <div className="w-full mt-auto pt-2 border-t border-slate-200">
          <div className="flex items-center gap-2 px-2 py-2">
            <AvatarCircle />
            <div className="font-['Lexend:Regular',_sans-serif] text-[14px] text-slate-900">Text content</div>
            <button
              type="button"
              className="ml-auto size-8 rounded-md flex items-center justify-center hover:bg-slate-100"
              aria-label="More"
            >
              <svg className="size-4" viewBox="0 0 16 16" fill="none">
                <circle cx="4" cy="8" r="1" fill="#0f172a" />
                <circle cx="8" cy="8" r="1" fill="#0f172a" />
                <circle cx="12" cy="8" r="1" fill="#0f172a" />
              </svg>
            </button>
          </div>
        </div>
    ...
    >
      <div
        className={`rounded-lg cursor-pointer transition-all duration-500 flex items-center relative ${
          item.isActive ? "bg-primary/10 [&_svg]:!text-primary" : "hover:bg-slate-50 [&_svg]:hover:!text-primary"
      ...
      >
        ...
        <div
          ...
          style={{ transitionTimingFunction: softSpringEasing }}
        >
          <div className={`font-['Lexend:Regular',_sans-serif] text-[14px] leading-[20px] truncate ${item.isActive ? "text-primary font-medium" : "text-slate-900 group-hover:text-primary"}`}>
            {item.label}
          </div>
        </div>
        ...
        {item.hasDropdown && (
          ...
          >
            <ChevronDownIcon
              size={16}
              className="text-slate-900 transition-transform duration-500"
              style={{
...
function SubMenuItem({ item, onItemClick }: { item: MenuItemT; onItemClick?: () => void }) {
  ...
  const handleClick = () => {
    ...
    } else if (item.isDanger && item.label === "Close Account") {
  ...
  return (
    <div className="w-full pl-9 pr-1 py-[1px]">
      <div
        className={`h-10 w-full rounded-lg cursor-pointer transition-colors group flex items-center px-3 py-1 ${item.isDanger ? "hover:bg-red-50" : (item.isActive ? "bg-primary/5" : "hover:bg-primary/5")}`}
        onClick={handleClick}
      >
        <div className="flex-1 min-w-0">
          <div className={`font-['Lexend:Regular',_sans-serif] text-[14px] leading-[18px] truncate ${item.isDanger ? "text-red-500 group-hover:text-red-700" : (item.isActive ? "text-primary font-medium" : "text-slate-600 group-hover:text-primary")}`}>
            {item.label}
          </div>
        </div>
        {item.isExternal && (
          <Share size={12} className="text-slate-400 group-hover:text-primary shrink-0 ml-2" />
        )}
      </div>
    </div>
  );
    ...
    <div className="flex flex-col w-full">
      ...
      >
        <div className="flex items-center h-10 px-4">
          <div className="font-['Lexend:Regular',_sans-serif] text-[14px] text-slate-500">
            {section.title}
          </div>
        </div>
      </div>
        ...
        return (
          <div key={itemKey} className="w-full flex flex-col">
            <MenuItem
              ...
              onItemClick={() => {
                if(item.label === "Close Account" && item.isDanger) {
                  if (window.confirm("Are you sure you want to close your account? This action cannot be undone.")) {
                    console.log("Account closure initiated.");
                  }
                } else {
